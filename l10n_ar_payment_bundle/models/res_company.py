...
class ResCompany(models.Model):
    ...
    def _create_payment_bundle_journal_if_needed(self):
        ...
        for company in companies:
            ...
            # If the database already has the journal configured for outbound, patch it with inbound
            outbound_id = company._get_bundle_journal("outbound")
            if outbound_id:
                in_method = self.env.ref("l10n_ar_payment_bundle.account_payment_in_payment_bundle", raise_if_not_found=False)
                if in_method:
                    journal = self.env["account.journal"].browse(outbound_id)
                    from odoo import Command
                    journal.write({
                        "inbound_payment_method_line_ids": [Command.create({"payment_method_id": in_method.id})]
                    })
                continue
